...
app.use(helmet({
  contentSecurityPolicy: {
    directives: {
      defaultSrc:  ["'self'"],
      scriptSrc:   ["'self'", "'unsafe-inline'", "https://accounts.google.com", "https://accounts.google.com/gsi/client", "https://apis.google.com", "https://cdn.jsdelivr.net", "https://cdnjs.cloudflare.com", "https://unpkg.com", "https://fonts.googleapis.com"],
      connectSrc:  ["'self'", "https://accounts.google.com", "https://accounts.google.com/gsi/", "https://apis.google.com", "https://www.google.com", "https://oauth2.googleapis.com"],
...
/* ================= FILE UPLOAD ================= */
const upload = multer({ dest: "uploads/", limits: { fileSize: 50 * 1024 * 1024 } }); // 50MB max
...
app.post("/api/chat", aiLimiter, verifyToken, async (req, res) => {
  try {
    ...
    const response = await fetch("https://api.anthropic.com/v1/messages", {
      ...
      body: JSON.stringify({ model: "claude-haiku-4-5-20251001", max_tokens: 600, system: systemContext || "You are a SOC AI assistant for FraudSys. Be concise.", messages: messages.slice(-20) })
    });
    const data = await response.json();
    if (!response.ok) return res.status(502).json({ error: data.error?.message || "Anthropic API error" });
...
/* ─────────────────────────────────────────────────────────────
   ...
   Returns counts only (no heavy document fetch) — fast!
───────────────────────────────────────────────────────────── */
app.get("/stats-summary", async (req, res) => {
  try {
    const uploadId = req.query.uploadId;
    const match    = (uploadId && uploadId.match(/^[a-f\d]{24}$/i))
    ...
    // Top flags
    const flagAgg = await Result.aggregate([
      { $match: match },
      { $unwind: "$flags" },
    ...
    res.json({
      ...
      avgConf:   counts?.avgConf   || 0,
      topFlags:  flagAgg.map(f => ({ flag: f._id, count: f.count }))
    });
...
app.post("/generate-data", uploadLimiter, async (req, res) => {
  try {
    const count     = Math.min(parseInt(req.body.count) || 5000, 50000); // max 50k free tier
    ...
    const CHUNK = 1000; // small chunks = less RAM
    ...
    for (let i = 0; i < count; i += CHUNK) {
      ...
      for (let j = 0; j < chunkSize; j++) {
        ...
        const rec = isFraud ? {
          amount: ri(800000,2500000), claims: ri(15,35), age: ri(22,65),
          policyDuration: ri(1,4), incidentType: rc(incidents),
          witnesses: ri(0,1), policeReport: Math.random() < 0.3,
          premiumAmount: ri(8000,25000), vehicleAge: ri(0,15), location: rc(cities)
        } : {
          amount: ri(50000,600000), claims: ri(1,8), age: ri(22,65),
          policyDuration: ri(12,60), incidentType: rc(incidents),
          witnesses: ri(2,5), policeReport: Math.random() < 0.85,
          premiumAmount: ri(15000,60000), vehicleAge: ri(0,15), location: rc(cities)
        };
    ...
    await logEvent("INFO", "UPLOAD", "Synthetic data generated", `${uploader} generated ${totalRows.toLocaleString()} rows`, uploader, req, { count: totalRows, fraudRate, totalMs });
    ...
    console.error("Datagen error:", err.message);
...
/* ─────────────────────────────────────────────────────────────
   RESULTS — capped at 5000, uses .select() for speed
───────────────────────────────────────────────────────────── */
app.get("/results", async (req, res) => {
  try {
    ...
    const results  = await Result.find(query)
      .select("amount claims age risk riskScore confidence anomaly flags location incidentType witnesses policeReport premiumAmount vehicleAge policyDuration uploadId uploadedAt")
      .sort({ uploadedAt: -1 })
      .limit(limit)
      .lean();
    res.json(results);
